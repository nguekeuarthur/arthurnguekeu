...
export function Particles() {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      // Create star particles (only in dark mode)
      if (theme === 'dark') {
        for (let i = 0; i < particleCount.stars; i++) {
          const starColors = ['#fff5f5', '#ebf8ff', '#fefcbf'];
          particles.push({
            x: Math.random() * canvas.width,
            y: Math.random() * canvas.height,
            dx: (Math.random() - 0.5) * animationSpeed * 0.5,
            dy: (Math.random() - 0.5) * animationSpeed * 0.5,
    ...
    function animate() {
      ...
      particles.forEach((particle, i) => {
        ...
        if (particle.type === 'star') {
          ...
          // Draw star center
          ctx.beginPath();
          ctx.arc(0, 0, particle.radius * 0.5, 0, Math.PI * 2);
